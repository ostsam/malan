...
// OPTIMIZATION: Memoized chat item component to prevent unnecessary re-renders
const ChatItem = React.memo(
  ({
    ...
    isRTLLanguage: (lang?: string | null) => boolean;
  }) => {
    ...
    const isRTL = useMemo(
      ...
      [chat.selectedLanguage, chat.nativeLanguage, isRTLLanguage]
    );

    const rtlStyles: React.CSSProperties = useMemo(
      () => ({
    ...
    const languageCode = useMemo(
      ...
      [chat.selectedLanguage, chat.nativeLanguage]
    );

    const handleMouseEnter = useCallback(() => {
      router.prefetch(`/chat/${chat.chatId}`);
    }, [router, chat.chatId]);

    return (
      <li className="group flex items-stretch justify-between border-b border-slate-300/60 dark:border-slate-600/60 last:border-b-0 py-1.5 hover:bg-[rgba(23,6,100,0.06)] dark:hover:bg-[rgba(23,6,100,0.12)] rounded-none transition-all duration-300 cursor-pointer mx-1">
        <Link
...
export default function AppSidebar() {
  ...
  const { pinnedChats, recentChats } = useMemo(
    () => ({
      pinnedChats: chatHistory.filter((chat) => chat.isPinned),
      recentChats: chatHistory.filter((chat) => !chat.isPinned),
    }),
    [chatHistory]
  );
  ...
  const handleUpdateSlug = useCallback(
    async (chatId: string, currentSlug: string) => {
  ...
  // OPTIMIZATION: Memoized chat list rendering
  const renderChatList = useCallback(
    ...
    [handleTogglePin, handleUpdateSlug, handleDeleteChat, isRTLLanguage]
  );
